...
class Game():
    def __init__(self, home_team, away_team, df, print=True, max_effective_time=900):
        ...
        self.DMs = {'run_pass': Heirarchical_DM(df, ['run_vs_pass', 'ydstogo'], 'yd_gain'),
                    'play_t_length': Distribution_Manager(df[(df.play_t_length > 0) & (df.play_t_length <= 40)], 'run_vs_pass', 'play_t_length')}
        self.PMs = {'run_pass': Predictor_Manager(df, 'yd_gain ~ 1 + time_left*goal_yd + ydstogo + run_vs_pass*pos_team + run_vs_pass*def_team'),
                    'play_t_length': Predictor_Manager(df[(df.play_t_length > 0) & (df.play_t_length <= 40)], 'play_t_length ~ 1 + run_vs_pass*pos_winning*time_left')}
        self.time_left = 3600
        ...
        self.basic_predictors = {'run_vs_pass': Basic_Predictor(df, 'run_vs_pass ~ pos_team + pos_winning*time_left*goal_yd + pos_minus_def_score*time_left + ydstogo*down',
                                                                logistic=True),
                                 ...
                                 'FG_made': Basic_Predictor(df[df['FG_attempted'] == 1],
                                                                 ...
                                                                 logistic=True),
                                 'fumble': Basic_Predictor(df, 'fumble ~ def_team + pos_team + run_vs_pass', logistic=True),
                                 ...
                                 'interception': Basic_Predictor(df, 'interception ~  pos_team + def_team', logistic=True),
                                 'int_yds': Basic_Predictor(df[df['interception'] == 1], 'int_spot_yds ~ def_team'),
                                 'penalty': Basic_Predictor(df, 'penalty ~ pos_team + def_team + goal_yd*ydstogo*down + pos_minus_def_score*time_left', logistic=True),
                                 'penalty_poss': Basic_Predictor(df[df['penalty'] == 1], 'penalty_pos ~ pos_team + def_team + goal_yd*ydstogo*down', logistic=True),
                                 'penalty_accepted': Basic_Predictor(df[df['penalty'] == 1], 'penalty_acpt ~  penalty_pos + down*ydstogo*goal_yd + pos_minus_def_score*time_left', logistic=True),
                                 'penalty_yds': Basic_Predictor(df.loc[(df['penalty'] == 1) & (df.yd_gain <= 15) & (df.yd_gain >= -15)], 'yd_gain ~ penalty_pos + goal_yd*down*ydstogo')}
    ...
    def simulate_play(self): # self.pos_team is receiving
        ...
        if run_pass:
            ...
            t_play = int(self.get_play_t_length(is_run) * 1.0) # *1.15 this lowers the number of plays. BIG SHAME!!!
        else:
            t_play = 10
    ...
    def check_and_do_punt(self):
        ...
        if self.ydline_100_pos > 100:
            self.move_yds(self.ydline_100_pos - 80, False, 'Touchback. Ball placed at 80 yard line')
        return is_punt
    ...
    def penalty_check(self):
        is_penalty = self.basic_predictors['penalty'].predict(pd.DataFrame.from_dict({'penalty': [None],
                                                                       'pos_team': [self.pos_team],
                                                                       'def_team': [self.def_team],
                                                                       'goal_yd': [self.ydline_100_pos],
                                                                       'ydstogo': [self.ydstogo],
        ...
        is_penalty = 0
        if is_penalty: 
            poss_team_penalty = self.basic_predictors['penalty_poss'].predict(pd.DataFrame.from_dict({'penalty': [None],
                                                                       'pos_team': [self.pos_team],
                                                                       'def_team': [self.def_team],
                                                                       'goal_yd': [self.ydline_100_pos],
                                                                       'ydstogo': [self.ydstogo],
            ...
            penalty_accepted = self.basic_predictors['penalty_accepted'].predict(pd.DataFrame.from_dict({'penalty_acpt': [None],
            ...
            if penalty_accepted:
                penalty_yds = 5 * (abs(self.basic_predictors['penalty_yds'].predict(pd.DataFrame.from_dict({'yd_gain': [None],
                                                                       'penalty_pos': [poss_team_penalty],
                                                                       'goal_yd': [self.ydline_100_pos],
                                                                       'ydstogo': [self.ydstogo],
                                                                       'down': [self.down],
                                                                       'time_left': [self.effective_time_left]})))/5)
                if poss_team_penalty:
...
if __name__ == '__main__':
    import numpy as np
    df = pd.DataFrame()
    fbs_conf = ['SEC', 'Mountain West', 'American Athletic', 'Pac-12', 'Mid-American', 'Sun Belt',  'ACC','Big 12', 'Big Ten','FBS Independents', 'Conference USA']
    for wk in range(1,14):
        nxt_wk = pd.read_csv(f'raw_data/2022_{wk}_pbp.csv',  engine='pyarrow')
        nxt_wk = nxt_wk.loc[(nxt_wk.offense_conference.isin(fbs_conf)) & (nxt_wk.defense_conference.isin(fbs_conf))]
    ...
    for row in new_games.index:
        ...
        'home_score': A_scores,
        'away_score': B_scores
        }.items() ])).fillna(method='ffill').to_csv(f'new_games/{TEAM_A}vs{TEAM_B}.csv', index=False)
